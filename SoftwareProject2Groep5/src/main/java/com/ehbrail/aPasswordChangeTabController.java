...
public class aPasswordChangeTabController implements Initializable {
	...
	@FXML
	private PasswordField copyNewPasswordField;
	@FXML
				...
				if (getScore() >= 4) {
					if (newPasswordField.getText().equals(copyNewPasswordField.getText())) {
						...
						if (changePassbyLogin(login, newHashPass)) {
							...
							Notifications.create().title(language.getString("succes"))
									.text(language.getString("notifSucces")).darkStyle().position(Pos.TOP_CENTER)
									.graphic(new ImageView(img)).show();
						} else
